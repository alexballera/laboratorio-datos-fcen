...
dniPersonasDesdeMadrid = dd.sql("""
                                ...
                                ON vuelosDesdeMadrid.Numero = r.NroVuelo
              """).df()
...
# a.- Vincular (JOIN)  EmpleadoRol y RolProyecto para obtener la tabla original EmpleadoRolProyecto
    # Aparece Tuplas espúrua, las tablas no se pueden unir y producen datos no ciertos,
    # para que no suceda debe tener la clave o super clave
consultaSQL = """
...
# Count cuentas las filas que coinciden con las condiciones
...
# AVG o MEAN para calcular promedio en vez de COUNT
    
consultaSQL = """
                SELECT Instancia, MEAN(edad) AS PromedioEdad
                FROM examen
                GROUP BY Instancia
                ORDER BY Instancia ASC
...
#   HAVING instancia='Parcial-01' OR instancia='Parcial-02' o HAVING instancia LIKE 'Parcial%'
consultaSQL = """
                SELECT Instancia, MEAN(nota) AS NotaPromedio
                FROM examen
                GROUP BY Instancia
...
promedioNotas = dd.sql("""
                SELECT Instancia, AVG(Nota) AS Promedio
                FROM examen
                GROUP BY Instancia
                """).df()

examenConPromedio = dd.sql("""
                    SELECT e.*, Promedio
                    FROM examen AS e
                    INNER JOIN promedioNotas AS dp
...
consultaSQL = """
                SELECT empleado, UPPER(rol) as Uppercase
                FROM empleadoRol
...
consultaSQL = """
                SELECT empleado, LOWER(rol) as lowercase
                FROM empleadoRol
